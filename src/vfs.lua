...
function vfs.newtmpvfs()
	local mountTable = {}
	local tmpvfs = {}
	function tmpvfs.getMountContainer(path)
		...
		if base == nil then
			error("Please mount / first before using tmpvfs",3)
		end
		return mountTable[base]
	end
	function tmpvfs.fake2real(path)
		path = vfs.normalize(path)
		local mT = tmpvfs.getMountContainer(path)
		return mT[1] .. (mT[2] == "/" and path or path:sub(#mT[2]+1))
	end
	function tmpvfs.isMountPath(path)
		path = vfs.normalize(path)
		for i = 1,#mountTable do
	...
	for i = 1,#quickPatch do
		tmpvfs[quickPatch[i]] = function(path,...)
			return love.filesystem[quickPatch[i]](tmpvfs.fake2real(path),...)
		end
	end
	...
	for i = 1,#copyOver do
		tmpvfs[copyOver[i]] = love.filesystem[copyOver[i]]
	end
	function tmpvfs.exists(filename)
		return tmpvfs.isMountPath(filename) or love.filesystem.exists(tmpvfs.fake2real(filename))
	end
	function tmpvfs.getDirectoryItems(dir)
		dir = vfs.normalize(dir)
		local result = love.filesystem.getDirectoryItems(tmpvfs.fake2real(dir))
	...
	function tmpvfs.getLastModified(filename)
		...
		return love.filesystem.getLastModifed(tmpvfs.fake2real(filename))
	end
	function tmpvfs.getSize(filename)
		...
		return love.filesystem.getSize(tmpvfs.fake2real(filename))
	end
	tmpvfs.init = function() end -- love.filesystem.init -- Don't call this, EVER
	function tmpvfs.isDirectory(filename)
	...
	function tmpvfs.newFileData(contents,name,decoder)
		...
		return love.filesystem.newFileData(tmpvfs.fake2real(contents))
	end
	function tmpvfs.remove(filename)
		if tmpvfs.isMountPath(filename) then
			return false
		end
		return love.filesystem.remove(tmpvfs.fake2real(filename))
	end
	tmpvfs.fsunmount = love.filesystem.unmount
	function tmpvfs.unmount(fakePath) -- Not the same as love.filesystem.unmount
